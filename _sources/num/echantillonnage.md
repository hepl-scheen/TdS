...
L'échantillonnage consiste à convertir un signal $x(t)$ analogique en une séquence (suite de nombres) $x[n] = x(nT_e)$ où $T_e$ est la « période d'échantillonnage ».
...
Cela revient a dire que l'on multiplie le signal $x(t)$ avec la fonction _peigne de Dirac_ (train d'impulsion).

$$ x^*(t) = x(t) \sum_k \delta(t-kT_e) $$

$$ x^*(t) = \sum_k x(kT_e) \delta(t-kT_e) $$

````{dropdown} Transformée de Fourier du peigne de Dirac.
